...
const scrapdata=async()=>{
  try {
    const response = await axios.post('http://localhost:10001/scrapdata', { weblink });
  ...
  const autofetchData = async (link) => {
    try {
      ...
      console.log("Error while auto fetching for links:", err);
    }
  };
  ...
  const autofetch = async () => {
    ...
    const intervalId = setInterval(async () => {
      if (index < links.length) {
        try {
          const result = await autofetchData(links[index]); 
            ...
            {urldetail.length > 0 && urldetail.map((detailArray, i) => (
              ...
              <tbody>
                ...
                <td style={{ padding: '0 !important' }}>
                  ...
                  <td style={{color:"#0b6bff", fontWeight:'bold'}}>{detailArray.newPrice-(detailArray.newPrice*Number(detailArray.offer)/100)}</td>
